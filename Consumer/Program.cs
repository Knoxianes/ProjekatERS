        ...
        static void GUI(Client client)
        {
            ...
            { // Svaki od kodova koji se salju su povezani sa kodovima na serverskoj strani
                case '1':
                    ...
                    break;
                case '2':
                    client.Send(Ukloniti_Potrosaca(), "3"); // Kod 3 brisanje potrosaca
                    client.WriteToFile("Uklonjen je potrosac.", "Log\\log_consumer.txt");
                    break;
                case '3':
                    client.Send(Izmeniti_Potrosaca(), "4"); // Kod 4 izmena posojeceg potrosaca
                    client.WriteToFile("Izmenjen je potrosac.", "Log\\log_consumer.txt");
                    break;
                case '4':
                    client.Send("", "5"); // Kod 5 ispis svih potrosca
                    client.WriteToFile("Poslat je zahtev za ispis svih potrosaca.", "Log\\log_consumer.txt");
                    break;
                case '5':
                    client.Send(Istampati_Potrosaca(), "6"); // Kod 6 ispis jednog potrosca
                    client.WriteToFile("Poslat je zahtev za ispis jednog potrosaca.", "Log\\log_consumer.txt");
                    break;
                case '6':
                    client.Send("", "7"); // Kod 7 gasenje svih programa
                    Console.WriteLine("\n * ****PROGRAM SE GASI!!! * ****");
